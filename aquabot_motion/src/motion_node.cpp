...
private:
  ...
  Eigen::VectorXd predict_state(const Eigen::VectorXd& state, const Eigen::VectorXd& control) {
    ...
    double tau_N = (L_ / 2.0) * ((F_R * std::cos(alpha_L)) - (F_L * std::cos(alpha_R))) - (W_ / 2.0) * ((F_L * std::sin(alpha_L)) + (F_R * std::sin(alpha_R)));
  ...
  void control_loop() {
    ...
    for(int k=0; k<K_; k++){
      ...
      for(int h=0; h<horizon_; h++){
        ...
        noise(1) *= 0.8;
        noise(2) *= 0.3; // angle noise variance
        noise(3) *= 0.3;

        Eigen::VectorXd noisy_control = u_nominal + noise;

        // clamp to physical limits so prediction doesn't explode
